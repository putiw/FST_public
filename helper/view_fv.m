...
for whichHemi = 1:numel(hemi)
    ...
    if ismember(whichSub,{'fsaverage','fsaverage6'})
        ...
        % 
        % cmd = sprintf('%s -f %s%s:label=%s:label_outline=1:label_color=blue:label_opacity=1:label=%s:label_outline=1:label_color=black:label_opacity=1',cmd,inflated,overlayCmd,mtlabel,fstlabel);
